...
(function(exports) {
	...
	if (typeof process !== "undefined") {
		try {
			var helper = 'galaxy-stack';
			stackHelper = require(helper);
		} catch (ex) {
	...
	function invoke(that, fn, args, idx, options) {
		...
		if (options.errbackIndex != null) {
			args[idx] = function(r) { callback(null, r); }
			args[options.errbackIndex] = function(e) { callback(e); }
		} else {
	...
	exports.star = function(fn, idx, arity) {
	...
	function makeUnstarror(i) {
		return eval("(function(fn, options, entering)" + unstarBody.replace(/function\s*F\(\)/, "function F(" + makeArgs(i) + ")") + ")");
	}

	exports.unstar = function(fn, idx, entering, arity) {
