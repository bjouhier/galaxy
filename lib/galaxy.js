...
(function(exports) {
	...
	var unstarTemplate = function unstar(fn, idx) {
		...
		var F = function() {
				...
				if (!cb) return future(F, arguments, idx);
	...
	function makeUnstarror(i) {
		return eval("(function(fn, idx)" + unstarBody.replace(/function\s*\(\)/, "function(" + makeArgs(i) + ")") + ")");
	}
