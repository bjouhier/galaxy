...
(function(exports) {
	...
	try {
		var stackHelper = require('galaxy-stack');
	} catch (ex) {}
				...
				while (g) {
					try {
						...
						if (val && val[0] === PENDING) {
							err = val[1];
							val = val[2];
							if (stackHelper && err && !err.__async__) err = fillError(err, g);
						}
	...
	function fillError(err, g) {
		if (!stackHelper) return err;
		var continuations = [];
	...
	function stackTrace(err) {
		var extra = err.__async__;
		if (!extra) return "";
		var rawStack = Object.getPrototypeOf(err).stack;
		var frames = [];
		for (var g = extra.generator; g; g = g.prev) {
			var frame = stackHelper.getStackFrame(g, extra.continuations[frames.length]);
			frames.push(frame);
		}
		var starredStack = frames.map(function(frame) {
		...
		return rawStack.substring(0, cut) + //
		'    <<< await stack >>>\n' + starredStack + //
